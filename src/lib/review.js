...
export function selectConcepts(mem, { time = 'all', level = 'all', text = '' } = {}) {
  ...
  const wantWrong = level === 'wrong' || /hay sai|sai đi sai|sai nhiều|\bsai\b/.test(t)
  const wantNew = level === 'new' || /chưa ôn|chưa học|mới học|chưa làm/.test(t)
  const wantNotMastered = level === 'notmastered' || /chưa thành thạo|chưa đạt|chưa vững|100/.test(t)
  const wantWeak = level === 'weak' || /yếu/.test(t)
